...
export class ArticleService {
  ...
  async create(userId: number, dto: CreateArticleDto) {
    ...
    // Update tags
    await this.tagService.addUniqueTags(dto.tagList);
  ...
  async update(userId: number, slug: string, articleData: any): Promise<IArticleRO> {
    ...
    // Update tags if tagList is provided in articleData
    if (articleData.tagList && Array.isArray(articleData.tagList)) {
      await this.tagService.addUniqueTags(articleData.tagList);
    }
